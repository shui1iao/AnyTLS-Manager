...
install_pkg() { if have apt-get; then apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y "$@"; elif have dnf; then dnf install -y "$@"; elif have yum; then yum install -y "$@"; else
	err "未找到包管理器，请手动安装：$*"
	exit 1
fi; }
ensure_deps() {
	local m=()
	for c in curl unzip grep sed systemctl; do have "$c" || m+=("$c"); done
	((${#m[@]} == 0)) || install_pkg curl unzip grep sed systemd coreutils
}
validate_port() { [[ "$1" =~ ^[0-9]+$ ]] && ((10#$1 >= 1 && 10#$1 <= 65535)); }
rand_port() { echo $((RANDOM % 55536 + 10000)); }
rand_pass() {
	local a s1 s2 r
	a=$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 14 || true)
	s1='-/@'
	r="${a}${s1:$((RANDOM % 3)):1}${s1:$((RANDOM % 3)):1}"
	echo "$r" | grep -o . | shuf | tr -d '\n'
}
urlencode() {
...
asset_regex() { case "$(uname -m)" in x86_64 | amd64) echo 'linux_amd64\.zip$' ;; *)
	err "暂只支持 amd64/x86_64，当前架构：$(uname -m)"
	exit 1
	;;
esac }
latest_asset() {
	...
	url="$(printf '%s' "$json" | grep 'browser_download_url' | cut -d '"' -f 4 | grep -E "$re" | head -n1 || true)"
	[[ -n "$url" ]] || {
		err "未找到 amd64/x86_64 的 anytls-go Release 资产"
		exit 1
	}
	echo "$url"
}
install_anytls() {
	ensure_deps
	local url tmp port password
	url="$(latest_asset)"
	tmp="$(mktemp -d)"
	info "下载 anytls-go：$url"
	curl -fL --retry 3 -o "$tmp/anytls.zip" "$url"
	unzip -q -o "$tmp/anytls.zip" -d "$tmp"
	local found
	found=$(find "$tmp" -type f -name 'anytls-server*' | head -n1)
	[[ -n "$found" ]] || {
		err "压缩包内未找到 anytls-server"
		exit 1
	}
	install -m 0755 "$found" "$BIN"
	rm -rf "$tmp"
	read -rp "请输入 anytls 监听端口（留空随机）: " port
	port=${port:-$(rand_port)}
	validate_port "$port" || {
		err "端口无效"
		exit 1
	}
	read -rp "请输入 anytls 密码（留空随机）: " password
...
display_config() {
	local port="$1" password="$2" ip pe tag
	ip=$(curl -fsS4 https://ipv4.icanhazip.com 2>/dev/null | tr -d '\n' || true)
	ip=${ip:-"<服务器IP>"}
	pe=$(urlencode_anytls_auth "$password")
	tag=$(urlencode VPS)
	say "------------------------------------------"
	say "anytls 当前配置"
	say "地址: ${GREEN}$ip${NC}"
	say "端口: ${GREEN}$port${NC}"
...
view_config() {
	...
	display_config "$(parse_port)" "$(parse_pass)"
}
modify_config() {
	[[ -f "$SERVICE_FILE" ]] || {
		err "anytls 未安装"
	...
	local port password
	read -rp "新端口（留空随机）: " port
	port=${port:-$(rand_port)}
	validate_port "$port" || {
		err "端口无效"
		return 1
	}
	read -rp "新密码（留空随机）: " password
	password=${password:-$(rand_pass)}
	write_service "$port" "$password"
	systemctl daemon-reload
...
check_update() {
	local r tmp
	r=$(curl -fsSL "$VERSION_URL" 2>/dev/null | head -n1 | tr -cd '0-9.' || true)
	[[ -n "$r" && "$r" != "$VERSION" ]] || {
		ok "脚本已是最新 v$VERSION"
		return
	}
...
menu() {
	...
	while true; do
		...
		say "${CYAN}============================================${NC}"
		say "${GREEN}仓库: github.com/shui1iao/AnyTLS-Manager${NC}"
		say "${GREEN}作者: shui1iao${NC}"
		say "${CYAN}============================================${NC}"
...
update-script)
	...
	check_update
	;;
-h | --help | help) echo "bash anytls.sh [install|modify|view|start|stop|restart|status|uninstall|update-script]" ;; *) menu ;; esac
